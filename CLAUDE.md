...
- `avatar/` — `Mannequin` (poseable **and** resizable capsule skeleton; capsules are the cloth
  ...
  size-chart importer — unit-tested), `poses` (**lookbook pose library** — `setPose` freezes a real GLB idle/walk clip frame / a procedural stance; garments re-settle. Poses no clip contains are authored as **IK targets** — where the hand or foot goes, in the body frame and in multiples of that limb's own length, so one authoring fits any body — plus torso tilts. `avatar/ik` solves the joint by the law of cosines with a pole for the bend direction; both the tilts and the hip drop are applied **absolutely from a recorded base**, because the static loop re-poses every frame and an additive offset accumulates), `bodyPresets` (**body-shape presets** —
  Runway/Curvy/Plus/Athletic/Petite/Tall shape-multiplier sets, in-range so colliders stay valid; pure
  `applyBodyPreset` is unit-tested), `fascinator` (**the occasion headpiece** — not a small hat: no crown, pinned to one side on a comb and canted off the skull, with a sprung feather spray that arcs rather than spikes and a sinamay loop), `earFlaps` (**the ushanka · deerstalker** — one flap shape on one hinge, hinged on the *ear* landmark the earrings use, with the crown through the brimless graded lathe and blocked outside the head because a lined winter hat is; `?flapWorn=<down|up>`), `strawHats` (**the boater · panama** — a blocked crown, a swept brim and a ribbon band, with the difference carried in the spec: stiffened sennit is dead flat with a turned edge and a drum crown, soft toquilla falls away and takes a centre dent), `brimless` (**the fez · kufi · pillbox** — one lathe and a table of blocks, in the millimetres a milliner works to; a *fitted* crown is graded to the measured head in girth **and** in height so the skull is enclosed, an unfitted one perches; plus the fez's tassel, which spills over one edge as a bundle), `shades` (**four sunglasses blocks** specced by the lens▫bridge millimetres off a real temple, with one outline function doing the teardrop and the swept corner), `neckwear` (**a tie · a bow tie · braces**, as the path and profile maths they are made of — a Catmull-Rom through the neck/chest/waist so a strap lies ON the body rather than cutting through the chest, the necktie's blade profile, and the measured torso front-depth the capsule radius does not give), `extremities` (**hand/foot frames off the rig** — a capsule ends at the wrist/ankle and says nothing about which way the hand or foot *points*; the rig's toe-base, middle-finger and fingertip bones do, and `Mannequin.extremities()` feeds them to `accessoryAnchors` so a shoe toes out with the foot and a glove runs down the fingers. Empty on the procedural body, where the limb axis is the fallback), `wornSizing` (**sizing rules for worn hardware** — watch case from the wrist, sock heights in cm up from the sole, the shared shoe/sock last, and the measured limb-joint fractions + radii), `accessories` (**shoes · belt · hat · bag + headwear & neckwear**
...
- `export/` — `exporters3d` (glTF/OBJ + **USDZ** for iOS AR Quick Look), `garmentPattern` (**real per-garment flat pattern**: unwraps the
  ...
  **per-edge seam allowance** (`offsetPolygonPerEdge` + `seamAllowancePerEdge` + `cutLine`) — a deep folded
  ...
  pack), `seamStress` (**will the seams hold** — the two independent limits, thread and needle-damaged fabric, the non-monotonic peak between them, and the colour ramp the overlay bakes), `easeMap` (**per-zone ease map** — clearance from the cloth to the body's own colliders at each landmark, both sides measured on the same live avatar so it stays consistent where a girth difference would not; reports the **minimum** as well as the mean, because a garment fails where it is tightest, plus `bodyRadiusAt`, the body's silhouette radius at a height), `drapeScore` (**the Cusick drape test on the garment** — the hem's enclosed area against the circle its own perimeter could enclose, with the body as the clamp disc, plus the **node count** of folds from a hysteresis walk that counts a fold once however many samples it spans), `consumption` (**fabric to buy** — the marker is the cutting-room plan, the purchase order is longer: `fabricConsumption` nests the panels then adds the end loss, the splices and a shrinkage allowance (dividing by 1−s, not multiplying by 1+s, which comes up short), and `bestBolt` picks a width by **re-nesting at each one** because a panel a little over half the bolt can make a narrower roll win outright. The cost sheet now charges the bought metres, not the marker's), `cost` (**landed cost sheet**: pure `costRollup` (fabric yield × price + thread + trims + labour +
...
`?accessories=<fascinator,ushanka,deerstalker,boater,panama,fez,kufi,pillbox,tie,bowtie,suspenders,socks,gloves,studs,watch,anklet,shoes,belt,hat,bag,beanie,cap,bucket,balaclava,scarf,gaiter,visor,cowboy,tophat,bowler,boonie,goggles,sunglasses>` (worn accessories, incl. **headwear & neckwear** placed by the head/neck frame; **sunglasses** = shades on the face frame that coexist with any hat (e.g. `sunglasses,cap`), in four blocks via `?sunglassesStyle=<wayfarer|aviator|round|cat-eye>` — `avatar/shades` specs each one by the lens▫bridge millimetres off a real temple and sizes it against the measured face width; **turban** = a domed crown cap + stacked cloth wraps, the wrap count set by `?turbanWraps=<2..8>`; **visor** = the open-crown sport band wearing the cap's parametric bill; **cowboy** = the western block — a cattleman-creased tall crown over a side-rolled brim (pure `avatar/cowboy.cowboyBrimLift`) + leather band & buckle; **tophat/bowler** = the formal blocked-felt pair (pure `avatar/formalHats` — the flared stovepipe with side-curled brim · the hard low dome with the all-round rolled edge); **boonie** = the soft field hat with the chin cord) ·
...
`?scarfTuck=1` (the **tuck-into-coat** worn-state — pure `tuckCentre`: the tails go down short, pulled toward centre-front + pressed against the chest, as if tucked inside a coat; the tucked tips are pinned so they stay put) ·
`?wrapPreset=<draped|parisian|infinity|blanket|ascot|tucked>` (**pre-tied wrap presets** — one-click named scarf looks that set an exclusive worn-state + a flattering width over the knot/double/blanket/pin mechanics; `avatar/wrapPresets`, also a picker in the scarf panel) ·
...
`?garment=helmet-liner&layers=ski-mask` (the **two-layer balaclava** — the liner + shell simulate as layered cloth, kept apart by the inter-garment repulsion; the same **headwear-over-headwear** layering works for any two head coverings — e.g. `?garment=beanie&layers=snood` — the outer rides over the inner, covered by `tests/headwearLayering`) ·
...
features — brows/eyes/lips) · `?beard=<stubble|moustache|goatee|full>` (**facial hair** — `avatar/beard` is a coverage field, not a patch: the **cheek line** runs from the sideburn down to the corner of the mouth and the cheek above it is bare, a moustache is no wider than the mouth, stubble is the full beard's map at a fraction of its length, and the shell sinks *under* the skin where nothing grows so the boundary needs no alpha) · `?freckles=<0..1>` (the **malar scatter** — `avatar/freckles` rejection-samples the butterfly distribution across the nose bridge and cheekbones, cut out at the shaded eye sockets, sized as real 1–4 mm ephelides and coloured off the current skin tone rather than a fixed brown) ·
...
piece) · `?prints=embroidery` / `?prints=applique` (a raised embroidered / appliqué motif) · `?prints=woven` (**digital print on the weave** — a flat digital textile print with a fine warp/weft hatch multiplied onto just the ink, so the cloth grain shows through and it reads as printed *on the fabric*, not a sticker; a Finish option too) · `?prints=enamel-pin` (the **enamel pin / patch designer** — a hard raised badge: a gold metal rim, glossy enamel fill in the print colour, an inlaid motif + a gloss highlight; opaque like appliqué; also a Finish option in the print editor) ·
...
`?sleeveShape=<set-in|raglan|dolman|bishop|puff|bell>` · `?collarStyle=<band|shirt|mandarin|peterpan|notch|peak|shawl>` (incl. the three tailored **lapel gorges** — notch · peak (points swept up) · shawl (smooth rounded roll)) · `?pleatStyle=<knife|box|accordion|cartridge|gather>` · `?hemShape=<high-low|shirttail|handkerchief|ear-flap|point-front>` (a curved hem — pure `bottomEdge` shapes the 3D fill AND the 2D unwrap) ·
...
`?probeAxes=1` (**bone axis audit** — rotates a bone a known amount about each of its local axes and reports where the limb's tip went; a rig's axes are its own and a wrong sign puts a hand through the chest) · `?poseDebug=1` (logs each solved limb: where it was asked to reach, where it got to, and by how much it missed) ·
...
`?seamStress=1` (**seam-stress overlay** — how hard the *seams* are working, which is a different question from the cloth: `export/seamStress` takes the weaker of the **thread limit** (rising with stitch density) and the **fabric limit** (falling with it, because every stitch is another needle hole), so seam strength *peaks* at a density `bestSpi` reports — which is why a sewing room specifies an SPI instead of "as many as possible") ·
