...
`?probeHead=1` / `?probeLimb=1` (**surface probes** — raycast the *rendered* avatar and log its head profile / limb radii against the colliders, so anything worn on the body is placed against measured geometry rather than an assumed ratio) ·
