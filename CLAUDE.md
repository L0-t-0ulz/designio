...
`?accessories=<shoes,belt,hat,bag,beanie,cap,bucket,balaclava,scarf,gaiter,visor,cowboy,tophat,bowler,goggles>` (worn accessories, incl. **headwear & neckwear** placed by the head/neck frame; **visor** = the open-crown sport band wearing the cap's parametric bill; **cowboy** = the western block — a cattleman-creased tall crown over a side-rolled brim (pure `avatar/cowboy.cowboyBrimLift`) + leather band & buckle; **tophat/bowler** = the formal blocked-felt pair (pure `avatar/formalHats` — the flared stovepipe with side-curled brim · the hard low dome with the all-round rolled edge)) ·
