...
export type ExportFormat = 'glb' | 'glb-draco' | 'usdz' | 'obj' | 'stl' | 'svg' | 'dxf' | 'plt' | 'plt-roll' | 'grade-nest' | 'pattern-tiled' | 'pattern-tiled-a3' | 'pattern-tiled-a2' | 'techpack' | 'json' | 'manufacture' | 'factory-json' | 'size-set' | 'listing' | 'product-page' | 'portfolio' | 'configurator' | 'tryon-widget' | 'separations' | 'bag-pattern' | 'shoe-pattern' | 'mobile'
...
export interface PanelOptions {
  ...
  /** Stretch-utilisation map — strain as a fraction of the fabric's own usable stretch. */
  utilisation?: { get: () => boolean; set: (on: boolean) => void }
  /** Wrinkle-density map — mean curvature of the draped surface. */
  wrinkleMap?: { get: () => boolean; set: (on: boolean) => void }
  /** Seam-stress overlay — how hard the SEAMS are working, not the cloth. */
  seamStress?: { get: () => boolean; set: (on: boolean) => void }
...
export function createControlPanel(opts: PanelOptions): { panel: HTMLElement; api: PanelApi } {
  ...
  const TEXTILE_LABELS: Record<TextilePattern, string> = {
    stripe: 'Stripe', plaid: 'Plaid', check: 'Check', gingham: 'Gingham', polka: 'Polka', camo: 'Camo', herringbone: 'Herringbone', houndstooth: 'Houndstooth', chevron: 'Chevron', argyle: 'Argyle', pinstripe: 'Pinstripe', windowpane: 'Windowpane', 'glen-check': 'Glen check', 'dot-grid': 'Dot grid', basketweave: 'Basketweave', 'diagonal-stripe': 'Diagonal stripe'
  }
  ...
  const WEAR_LABELS: Record<WearKind, string> = { faded: 'Faded', 'acid-wash': 'Acid-wash', distressed: 'Distressed', adaptive: 'Adaptive', 'stone-wash': 'Stone-wash', 'enzyme-wash': 'Enzyme-wash' }
  ...
  const utilT = opts.utilisation ? toggle({ label: 'Stretch utilisation', get: () => opts.utilisation!.get(), set: (v) => { opts.utilisation!.set(v); syncStrainToggles() } }) : null
  const wrinkT = opts.wrinkleMap ? toggle({ label: 'Wrinkle density', get: () => opts.wrinkleMap!.get(), set: (v) => { opts.wrinkleMap!.set(v); syncStrainToggles() } }) : null
  const seamT = opts.seamStress ? toggle({ label: 'Seam stress', get: () => opts.seamStress!.get(), set: (v) => { opts.seamStress!.set(v); syncStrainToggles() } }) : null
  ...
  const utilisationRow = utilT ? utilT.row : el('div')
  const wrinkleMapRow = wrinkT ? wrinkT.row : el('div')
  const seamStressRow = seamT ? seamT.row : el('div')
  ...
  sceneGroup.append(redrapeRow, heatmapRow, stressRow, pressureRow, utilisationRow, wrinkleMapRow, seamStressRow, wrinkleRow, tearRow, slipRow, opts.sim ? simSec.root : el('div'), env.root, animSec.root, opts.timeline ? tlSec.root : el('div'), view.root)
