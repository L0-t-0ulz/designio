...
- [x] **Thermochromic preview** — a heat-reactive dye finish: pure `thermochromicColor` blends the cold base → warm-activated colour by a `thermoTemp` (0…1), with `thermoDefaultWarm` giving the classic leuco-dye *pale fade* when no second colour is set. Opt-in `thermo` finish applied in the stack over the fabric/finishes (at temp 0 nothing changes, so the cold state + per-part colours stand). `?thermo=1&thermoTemp=&thermoWarm=`; persisted on the layer + colorway. Unit-tested (blend + clamp + pale-fade default + colorway/`.dio` round-trip) + visually confirmed a denim dress fading pale as it warms — PR #TBD
