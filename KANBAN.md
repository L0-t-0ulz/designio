...
- [x] **Repeat-pattern engine** — textile repeat *layouts*: pure `repeatCell(mode, cx, cy)` gives each cell's tile offset + mirror flags, `repeatSuperTiles` the seamless super-tile size; `paintTextile` composes the base tile into that super-tile so a pattern tiles as a straight grid (**full-drop**), a staggered **half-drop** (alternate columns dropped half a tile), a **half-brick** (alternate rows shifted), or a **mirror** book-match that hides the seam. `?textileRepeat=<mode>`; threaded through the layer/colorway/config like `textileScale`. Unit-tested (per-mode offsets/flips · negative-index parity · colorway/`.dio` round-trip) + visually confirmed staggered vs straight polka dots — PR #TBD
