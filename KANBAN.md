...
- [ ] **Long-sleeve drape on the procedural body (HIGH)** — a long sleeve juts out ~45–90° (worst on the blazer's contrast sleeve) instead of hanging down the (A-posed) arm; the jut is **asymmetric** (one sleeve worse than the other) but **deterministic**, and it hits **every** long-sleeve garment (top · long-sleeve · hoodie · coat · blazer). Full audit trace:
  ...
  - **Likely real fix (armhole side of the BODY tube):** cut/shape a real **armhole** at the sides where the sleeve attaches (so the side fabric doesn't span the arm gap), and/or support the shoulder-line side row against the shoulder/deltoid capsule. Bigger than a one-liner (mesh topology) — its own careful pass + golden re-verify (the golden dress is short-sleeve but a full-shoulder bodice, so re-verify carefully, or bless the CI `golden-out` artifact).
