...
- [ ] **Long-sleeve drape on the procedural body (HIGH)** — a long sleeve juts out ~45–90° (worst on the blazer's contrast sleeve) instead of hanging down the (A-posed) arm; the jut is **asymmetric** (one sleeve worse than the other) but **deterministic**, and it hits **every** long-sleeve garment (top · long-sleeve · hoodie · coat · blazer). Full audit trace:
  ...
  - **Leading hypothesis:** the sleeve cap spawns right at the shoulder/torso capsule junction (cap at `x≈−0.14, y≈1.43` sits inside the torso cap radius `0.15` + on the shoulder bar). The **pinned** top ring is frozen there (inside the body), but the adjacent **unpinned** rings get **ejected outward** by the body-collision push-out → the sleeve splays horizontally. The thin arm + wide sleeve amplify it.
  - **Likely real fix:** spawn the sleeve rings just **outside** the arm/torso colliders (never inside), and/or pin the sleeve at **multiple rings down the arm** (shoulder · elbow · cuff → arm anchors) so it's held to the arm and can't be ejected — not a one-liner; needs its own careful pass + golden re-verify. Golden-safe path exists (golden scenes are short-sleeve/sleeveless; the 15 cm forearm-pin guard already skips short sleeves).
