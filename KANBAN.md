...
- [x] **Durag** — a data-driven **`durag`** garment: the silky skull wrap — a snug open-face crown cap in **satin** with a long flap draping the nape, via a new **`back-flap`** hem shape (pure `bottomEdge` drops the centre-back edge, mirroring `point-front`). Additive; unit-tested (back-flap drops the back not the front/sides, deeper than a front-point · durag is open-face satin with the back-flap default) + sims stably + visually confirmed the cap + nape flap from the rear — PR #391 _(the long tie tails are a follow-up; the cap + flap is the worn silhouette)_
...
- [x] **Satin-lined bonnet** — a data-driven **`bonnet`** garment: the beanie's gathered-crown headTube block worn loose in **satin** (`defaultFabric: 'satin'`) with **trapped-air loft** (`puff`) standing the dome roomy off the head — the classic satin hair bonnet, gathered top + snug edge at the hairline. Additive registry entry; unit-tested (crown headwear · satin + puff defaults · beanie-fit) + sims stably in the golden stability suite + visually confirmed the gathered satin dome — PR #389 _(the dedicated elastic-band edge ring is its own card, Elastic band physics)_
...
- [x] **Waxed rain hat** — a `waxed-cotton` fabric: a heavy plain-weave cotton paraffin-waxed for rain — stiff, water-repellent, with a **partial clearcoat** (a low waxed semi-gloss, not patent's mirror lacquer) over a still-cottony roughness, in a dark waxed olive (mirrors how patent leather shipped via the `clearcoat` field). Composes with the care label: `headwearMaterial` already routes `/wax/` → **coated**, so a waxed hat's care is *wipe clean · re-proof*. Unit-tested (partial clearcoat < patent, smoother than raw canvas, stiff shell); purely additive — PR #385 _(the fine water-bead micro-normal deferred, like patent's crackle normal)_
...
- [x] **Reflective piping** — an opt-in `reflectiveTrim` finish: pure `reflectiveTrimLook` (the trim colour lifted toward white for a self-lit glow over a low, room-catching roughness) applied to the trim material so the piping reads as bright hi-vis reflective tape. Headwear has no body-tube hem/neck band, so `buildDecor` now also rings the **head-tube band edge** — the piping shows on a beanie/cap. `?reflectiveTrim=1`; persisted on the layer + colorway; undefined-default so nothing else changes. Unit-tested (recipe + colorway/`.dio` round-trip) + visually confirmed the glowing hi-vis band on a melton beanie — PR #388
- [x] **Headwear pattern suite** — cloth-sim headwear produced **no** flat pattern (`garmentToPanels` ignored `specs.head`). Now each head tube unwraps into real millinery panels: a **gored crown** (pure `unwrapCrownGore` — a symmetric wedge whose height is the true *slant* length down the profile and whose width at each level is that level's circumference split N ways, cut 6) + a **band** strip (`headBandStrip`), plus a **brim** crescent (`headBrimPanel` — the front half-annulus unrolled) for a visored style; a neck cowl/snood/gaiter unwraps front/back like a body tube. Exports through the existing SVG/DXF path. Unit-tested (gore ≈ 1/6 of the band, brim on visored, cowl front/back on neck, SVG/DXF carry the panels, body garments unchanged) + visually confirmed the gore/band/brim in the 2D pattern view — PR #387
...
- [x] **Thermochromic preview** — a heat-reactive dye finish: pure `thermochromicColor` blends the cold base → warm-activated colour by a `thermoTemp` (0…1), with `thermoDefaultWarm` giving the classic leuco-dye *pale fade* when no second colour is set. Opt-in `thermo` finish applied in the stack over the fabric/finishes (at temp 0 nothing changes, so the cold state + per-part colours stand). `?thermo=1&thermoTemp=&thermoWarm=`; persisted on the layer + colorway. Unit-tested (blend + clamp + pale-fade default + colorway/`.dio` round-trip) + visually confirmed a denim dress fading pale as it warms — PR #392
...
- [x] **Unisex / gender-neutral blocks** — a **`unisex`** body-shape preset: a straighter, less-gendered silhouette (flatter chest · a less-nipped/straighter waist · narrower hips) so the same design reads unisex, applied on top of either figure like the other presets. Additive `BODY_PRESETS` entry (auto-flows to the preset picker + `?bodyPreset=unisex`); in-range so colliders stay valid. Unit-tested (flatter chest + straighter waist than curvy · in slider range · distinct) + visually confirmed the straighter block — PR #398
...
- [x] **Repeat-pattern engine** — textile repeat *layouts*: pure `repeatCell(mode, cx, cy)` gives each cell's tile offset + mirror flags, `repeatSuperTiles` the seamless super-tile size; `paintTextile` composes the base tile into that super-tile so a pattern tiles as a straight grid (**full-drop**), a staggered **half-drop** (alternate columns dropped half a tile), a **half-brick** (alternate rows shifted), or a **mirror** book-match that hides the seam. `?textileRepeat=<mode>`; threaded through the layer/colorway/config like `textileScale`. Unit-tested (per-mode offsets/flips · negative-index parity · colorway/`.dio` round-trip) + visually confirmed staggered vs straight polka dots — PR #396
- [ ] **Print warp-to-seamline** — a placement print that wraps cleanly across seams
- [x] **Duotone** print effect — a two-tone remap of the fabric/pattern onto a shadow→highlight ramp (the classic duotone / risograph look): pure `duotonePalette` (5 presets — noir · sepia · cyanotype · acid · blush), `duotoneMap` (luminance→ramp) + Rec-709 `luminance`; `paintDuotone` bakes it into the albedo **behind the prints** (so logos stay full-colour) in `buildDesignArt`, after the textile/tartan/colourwork ground. `?duotone=<kind>`; persisted on the layer + colorway; counts toward `hasArt`. Unit-tested (ramp endpoints/clamp · monotonic · every preset dark<light · Rec-709 · colorway/`.dio` round-trip) + visually confirmed a camo dress remapped to a cyanotype blueprint — PR #395 _(the gradient-mesh half is a follow-up)_
- [x] **Puff / discharge / foil** print finishes — three new `PrintStyle`s on the placed-print system: **puff** (a chunky rounded high-loft — the motif painted full-white with a soft blur so the bump domes like a rubber puff, and `printIsRaised` flags it so the material binds the bump map), **discharge** (bleaches the fabric where it prints — `screen`-composited a pale motif tone), **foil** (a bright metallic transfer via the pure `foilTone` — the print hue lifted vivid + bright). `?prints=puff|discharge|foil` demos. Unit-tested (puff raised · discharge/foil flat · foilTone always brighter than its base) + visually confirmed a domed PUFF, a bleached FADE on denim, and a bright gold foil — PR #397 _(true per-region metalness for foil is a material-level follow-up)_
