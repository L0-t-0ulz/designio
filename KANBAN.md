...
- [x] **Durag** — a data-driven **`durag`** garment: the silky skull wrap — a snug open-face crown cap in **satin** with a long flap draping the nape, via a new **`back-flap`** hem shape (pure `bottomEdge` drops the centre-back edge, mirroring `point-front`). Additive; unit-tested (back-flap drops the back not the front/sides, deeper than a front-point · durag is open-face satin with the back-flap default) + sims stably + visually confirmed the cap + nape flap from the rear — PR #TBD _(the long tie tails are a follow-up; the cap + flap is the worn silhouette)_
