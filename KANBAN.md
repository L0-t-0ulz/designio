...
- [x] **Corduroy cap fabric** — a `corduroy-cap` preset: a lighter (280 gsm) mid-wale cotton cord scaled for a cap's small panels — fewer, chunkier wales (`weaveScale` 40 vs the coat cord's 70) so the wale still reads at hat framing instead of blurring flat, in a classic camel, on the same vertical-cord pile weave + anisotropic sheen. Purely additive; unit-tested (lighter + chunkier-wale than the coat corduroy, same pile weave) — PR #378 _(visual wale scale to confirm in the next capture sweep)_
