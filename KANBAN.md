...
- [x] **Waxed rain hat** — a `waxed-cotton` fabric: a heavy plain-weave cotton paraffin-waxed for rain — stiff, water-repellent, with a **partial clearcoat** (a low waxed semi-gloss, not patent's mirror lacquer) over a still-cottony roughness, in a dark waxed olive (mirrors how patent leather shipped via the `clearcoat` field). Composes with the care label: `headwearMaterial` already routes `/wax/` → **coated**, so a waxed hat's care is *wipe clean · re-proof*. Unit-tested (partial clearcoat < patent, smoother than raw canvas, stiff shell); purely additive — PR #382 _(the fine water-bead micro-normal deferred, like patent's crackle normal)_
